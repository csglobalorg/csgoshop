...
try {
  ...
  global.products = [
    { id: '1', name: 'Test Product 1', price: 100, originalPrice: 150, category: 'Tech', isTrending: true, badge: 'new', rating: 5, sales: 100 }
  ];
  global.categories = [{ name: 'Tech', displayName: 'Tech', icon: 'fas fa-laptop' }];
  global.mainContent = document.getElementById('main-content');
  
  // Intercept the interpolation
  let originalGenerateProductsHtml = generateProductsHtml;
  generateProductsHtml = function(arr) {
     console.log("Called with array of length:", arr.length);
     return originalGenerateProductsHtml(arr);
  };
